<template>
  <section class="container bk cover page">
    <div class="content row">
      <div class="left-box">
        <h4 class="title">
          <strong><i class="icon ali-icon-community"></i>消息中心</strong>
        </h4>
        <ul class="list">
          <li
            ...
            :key="key"
            v-ripple
            :class="{ active: type === key, settings: key === `settings` }"
            @click="$router.push(key)"
          >
            <i v-if="key === `settings`" class="icon ali-icon-settings"></i>
            {{ item }}
            <span
              v-if="key !== `settings` && $store.state.message[key + `Count`]"
              class="message-count"
            >
