<template>
  <transition name="fade" :enter-active-class="enterActiveClass" :leave-active-class="leaveActiveClass"
    ...
    <div class="year short-duration" v-show="visible">
      <div v-for="item in yearList" :key="item.value" class="year-item">
        <Btn flat round @click="changeDate(item.value)" :color="item.active?`primary`:`default`">{{item.value}}</Btn>
      </div>
    </div>
  </transition>
</template>
...
<script>
  ...
  export default {
    ...
    computed: {
      yearList() {
        let yearList = [];
        let thisYear = this.calendar.getFullYear();
