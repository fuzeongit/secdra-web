<template>
  <transition name="fade" :enter-active-class="enterActiveClass" :leave-active-class="leaveActiveClass"
    ...
    <div class="calendar short-duration" v-show="visible">
  ...
  export default {
    ...
    computed: {
      ...
      calendarDataList() {
        ...
        let firstDayDate = this.firstDayDate;
        let lastDayDate = this.lastDayDate;
        for (let i = 0; i < lastDayDate.getDate(); i++) {
          let date = dateUtil.addDay(firstDayDate, +i);
          let active = date.getFullYear() === this.date.getFullYear() && date.getMonth() === this.date.getMonth() && date.getDate() === this.date.getDate();
          calendarDataList.push(new Calendar(date.getFullYear(), date.getMonth() + 1, date.getDate(), date, active));
        }
    ...
    methods: {
      changeDate(day) {
        this.date = new Date(this.calendar.getFullYear(), this.calendar.getMonth(), day);
